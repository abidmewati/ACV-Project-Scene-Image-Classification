...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    for images, labels in loader:
        ...
        # Standard loss calculation for VGG-16
...
def train_model(model, model_name, train_loader, val_loader, device, start_epoch=0, total_epochs=80, learning_rate=0.001):
    print(f"\n{'='*50}")
    print(f"Resuming Training {model_name} from epoch {start_epoch}")
    print(f"{'='*50}")
    ...
    # Note: If you want to resume the optimizer state exactly, 
    # you would need to save/load optimizer.state_dict() as well.
    # For now, we will use the current optimizer state.
...
if __name__ == "__main__":
    ...
    # 2. Initialize VGG-16
    vgg16 = models.vgg16(weights=None) # Set weights to None because we will load our own
    vgg16.classifier[6] = nn.Linear(in_features=4096, out_features=NUM_CLASSES)
    
    # --- ADD THIS TO LOAD YOUR CHECKPOINT ---
    checkpoint_path = os.path.join(SAVE_PATH, "VGG16_Baseline.pth")
    if os.path.exists(checkpoint_path):
        print(f"Loading checkpoint from {checkpoint_path}...")
        vgg16.load_state_dict(torch.load(checkpoint_path))
        start_epoch = 20 # You finished 20, so next is 20 (index starts at 0, so this makes sense)
    else:
    ...
    results = train_model(
        ...
        device=device,
        start_epoch=start_epoch, # Pass the variable here
        total_epochs=40,
        learning_rate=LEARNING_RATE
    )

    # 4. Plot Metrics
    plot_results(results, "VGG16")