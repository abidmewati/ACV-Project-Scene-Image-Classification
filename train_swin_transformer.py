...
def get_data_loaders(train_path, test_path, batch_size):
    ...
    # num_workers=0 is recommended for local Windows execution
...
def train_model(model, model_name, train_loader, val_loader, device, start_epoch=0, total_epochs=40, learning_rate=0.001):
    print(f"\n{'='*50}")
    print(f"Resuming Training {model_name} from epoch {start_epoch + 1}")
    print(f"{'='*50}")
    ...
    # Start with your last known best validation accuracy from Epoch 13
    best_val_acc = 82.94 
...
if __name__ == "__main__":
    ...
    # 2. Initialize Swin Transformer (Tiny)
    swin_t = models.swin_t(weights=None) # Set to None because we are loading your saved file!
    num_ftrs = swin_t.head.in_features 
    swin_t.head = nn.Linear(in_features=num_ftrs, out_features=NUM_CLASSES)

    # --- CHECKPOINT LOADING LOGIC ---
    checkpoint_path = os.path.join(SAVE_PATH, "SwinTransformer_Baseline.pth")
    if os.path.exists(checkpoint_path):
        print(f"Successfully found checkpoint: {checkpoint_path}")
        swin_t.load_state_dict(torch.load(checkpoint_path))
        start_epoch = 21 # Since you finished epoch 21, we start at index 21 (which is Epoch 22)
    else:
    ...
    results = train_model(
        model=swin_t,
        model_name="SwinTransformer_Baseline",
    ...
    # 4. Plot Metrics
    plot_results(results, "SwinTransformer")