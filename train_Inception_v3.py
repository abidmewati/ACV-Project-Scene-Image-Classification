...
def get_data_loaders(train_path, test_path, batch_size):
    # NOTE: Inception V3 natively expects 299x299 input images!
...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    for images, labels in loader:
        ...
        # Handle InceptionV3 returning a tuple (outputs, aux_outputs) during training
        if isinstance(outputs, tuple):
            primary_outputs, aux_outputs = outputs
            # Standard practice for InceptionV3: loss = primary_loss + 0.4 * aux_loss
            loss1 = criterion(primary_outputs, labels)
            loss2 = criterion(aux_outputs, labels)
            loss = loss1 + 0.4 * loss2
            
            # Use primary outputs for accuracy calculation
...
def validate_one_epoch(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for images, labels in loader:
            ...
            # In evaluation mode, InceptionV3 only returns the primary outputs
            if isinstance(outputs, tuple):
                outputs = outputs[0]
...
if __name__ == "__main__":
    ...
    # 2. Initialize Inception V3
    inception_v3 = models.inception_v3(weights=models.Inception_V3_Weights.DEFAULT, aux_logits=True)
    
    # Replace the classification heads for Inception V3
    # Main classifier (2048 features in)
    inception_v3.fc = nn.Linear(in_features=2048, out_features=NUM_CLASSES)
    
    # Auxiliary classifier (768 features in)
    inception_v3.AuxLogits.fc = nn.Linear(in_features=768, out_features=NUM_CLASSES)

    # 3. Train Model
    results = train_model(
        model=inception_v3,
        model_name="InceptionV3_Baseline",
    ...
    # 4. Plot Metrics
    plot_results(results, "InceptionV3")